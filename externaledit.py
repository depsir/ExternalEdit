	...
	def on_load(self,view):
		...
		if re.search(r'index( *\([0-9]*\))*.php$',doc):
			...
			if view.find('[Process]',0) and \
				...
				ee.site = wiki.Wiki(wk_server+wk_path+"/api.php") 
				pagename = re.search('title=([^&]*)', wk_url)
				pagename = pagename.group(1)
				view.set_name(pagename)
				# print pagename
				ee.mypage = page.Page(ee.site, pagename)
				# print mypage.getWikiText()
				edit = view.begin_edit()
				reg = sublime.Region(0,view.size())
				view.erase(edit,reg)
				view.set_syntax_file('Packages/Mediawiki.tmbundle/Syntaxes/Mediawiki.tmLanguage')
				view.set_encoding('utf-8')
				view.insert(edit,0,ee.mypage.getWikiText().decode('utf-8'))
				view.end_edit(edit)
	# def on_pre_save(self, view):
		# params = {}
		# params['text'] = view.substr(sublime.Region(0,view.size()))
		# global site
		# global mypage
		# site.login('Sacchi Francesco', 'aA!tIrih1')
		# mypage.edit(params)
		# print "saved"
